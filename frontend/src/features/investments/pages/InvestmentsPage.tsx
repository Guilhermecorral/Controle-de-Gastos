...
function CorporateEventPilotPanel() {
  ...
  const actionableEvents = [...newEvents, ...ambiguousEvents];
  ...
  return <div className="w-full">
    ...
    {(previewMutation.data || error) && <div className="mt-3 w-full rounded-[20px] border border-amber-200 bg-amber-50/70 p-4 text-sm text-slate-700">
      ...
      {actionableEvents.length > 0 && <div className="mt-4 space-y-2">{actionableEvents.map((event) => <label key={event.sourceReference} className={`flex gap-3 rounded-xl border p-3 ${event.status === 'NOVO' ? 'border-emerald-200 bg-white' : 'border-rose-200 bg-rose-50/70'}`}><input className="mt-1" type="checkbox" checked={selectedReferences.includes(event.sourceReference)} disabled={event.status !== 'NOVO'} onChange={() => toggle(event.sourceReference)} /><span className="min-w-0 flex-1"><span className="flex flex-wrap items-center gap-2"><b>{event.symbol || 'Ticker não identificado'}</b><span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${event.status === 'NOVO' ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'}`}>{event.status === 'NOVO' ? 'Nova previsão' : 'Vinculação pendente'}</span></span><span className="mt-1 block text-xs text-slate-500">{event.eventType === 'JCP' ? 'JCP' : event.eventType === 'RENDIMENTO' ? 'Rendimento' : 'Dividendo'} · Data Com {formatDate(event.exDate)} · Pagamento {formatDate(event.paymentDate)}</span><span className="mt-1 block text-xs text-slate-600">{event.status === 'NOVO' ? `${formatQuantity(event.quantityEligible)} cotas · ${currency(event.netAmount)} líquido previsto${event.eligibilityStartDate ? ` · na carteira desde ${formatDate(event.eligibilityStartDate)}` : ''}` : event.reason}</span></span></label>)}</div>}

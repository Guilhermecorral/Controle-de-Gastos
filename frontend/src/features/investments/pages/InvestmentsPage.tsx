...
function CorporateEventPilotPanel() {
  ...
  const loadPreview = () => {
    ...
    previewMutation.mutate(undefined, {
      onSuccess: (result: CorporateEventPilotPreviewResponse) => setSelectedReferences(result.events.filter((event) => event.status === 'VALIDO').map((event) => event.sourceReference)),
  ...
  return <div className="w-full">
    ...
    {(previewMutation.data || error) && <div className="mt-3 w-full rounded-[20px] border border-amber-200 bg-amber-50/70 p-4 text-sm text-slate-700">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-semibold text-amber-900">Agenda de Proventos Experimental</p><p className="mt-1 max-w-2xl text-xs leading-5 text-amber-800">Fonte ativa: {providerLabel}. A elegibilidade começa na primeira compra registrada do ativo. Confira valor, Data Com e pagamento antes de considerar o recebimento. Nenhuma receita é criada automaticamente.</p></div><span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-amber-800">{validCount} previsão(ões) válida(s)</span></div>

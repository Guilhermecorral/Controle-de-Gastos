...
function CorporateEventPilotPanel() {
  ...
  const existingCount = preview.filter((event) => event.status === 'JA_NA_AGENDA').length;
  ...
  return <div className="w-full">
    ...
    {(previewMutation.data || error) && <div className="mt-3 w-full rounded-[20px] border border-amber-200 bg-amber-50/70 p-4 text-sm text-slate-700">
      ...
      {(ineligibleEvents.length > 0 || existingCount > 0) && <details className="mt-4 rounded-xl border border-slate-200 bg-white/60 p-3"><summary className="cursor-pointer text-xs font-semibold text-slate-700">Diagnóstico da atualização: {ineligibleEvents.length} sem direito na Data Com e {existingCount} já na Agenda</summary><p className="mt-2 text-xs leading-5 text-slate-600">Itens sem direito não são erro: a posição não existia ou não tinha cotas na Data Com. Itens já publicados, confirmados ou em revisão também não são recriados.</p>{ineligibleEvents.length > 0 && <div className="mt-2 max-h-32 space-y-1 overflow-y-auto pr-1">{ineligibleEvents.slice(0, 30).map((event) => <p key={event.sourceReference} className="text-xs text-slate-500"><b>{event.symbol || event.assetName}</b> · Data Com {formatDate(event.exDate)} · {event.reason}</p>)}</div>}</details>}

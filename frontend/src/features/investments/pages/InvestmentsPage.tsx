...
import {
  ...
  useBatchWalletEarningActionMutation,
...
export default function InvestmentsPage() {
  ...
  const distribution = positions.reduce<Record<InvestmentAssetType, number>>((totals, position) => {
    ...
    return totals;
  }, { ACAO: 0, FII: 0, FIAGRO: 0, BDR: 0, ETF: 0, CRIPTO: 0, RENDA_FIXA: 0 });
...
function IncomeCalendar({ schedules, automaticEarnings, loading, onAdd }: { schedules: InvestmentIncomeScheduleResponse[]; automaticEarnings: WalletEarningResponse[]; loading: boolean; onAdd: () => void }) {
  ...
  const batchEarningMutation = useBatchWalletEarningActionMutation();
  const [error, setError] = useState('');
  ...
  const [batchRevertingEarnings, setBatchRevertingEarnings] = useState<WalletEarningResponse[]>([]);
  const [batchCancellingEarnings, setBatchCancellingEarnings] = useState<WalletEarningResponse[]>([]);
  const [batchConfirmingEarnings, setBatchConfirmingEarnings] = useState<WalletEarningResponse[]>([]);
  ...
  const visibleSchedules = showHistory ? [] : pending;
  const visibleAutomaticEarnings = showHistory ? [] : automaticPending;
  ...
  return <>
    ...
    <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
      ...
      <div className="flex flex-wrap gap-2"><CorporateEventPilotPanel />{historyCount > 0 && <div className="flex rounded-full border border-slate-200 bg-slate-50 p-1"><button className={`rounded-full px-3 py-1.5 text-sm font-semibold ${!showHistory ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`} type="button" onClick={() => setShowHistory(false)}>Agenda</button><button className={`rounded-full px-3 py-1.5 text-sm font-semibold ${showHistory ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`} type="button" onClick={() => setShowHistory(true)}>Histórico ({historyCount})</button></div>}<button className="rounded-full border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 hover:border-emerald-300 hover:text-emerald-700" type="button" onClick={onAdd}><Plus className="mr-1 inline" size={15} /> Agendar manualmente</button></div>
    </div>
    ...
    <ConfirmationDialog open={batchRevertingEarnings.length > 0} title="Desfazer recebimentos selecionados" description={`Isso removerá ${batchRevertingEarnings.length} receita(s) do Histórico Financeiro, seus movimentos vinculados e reflexos fiscais. As previsões voltarão para a Agenda para nova revisão.`} confirmLabel="Desfazer selecionados" busy={batchEarningMutation.isPending} onClose={() => setBatchRevertingEarnings([])} onConfirm={() => {
      setError('');
      batchEarningMutation.mutate({ earningIds: batchRevertingEarnings.map((earning) => earning.id), action: 'REVERT' }, {
        onSuccess: () => {
          setNotice(`${batchRevertingEarnings.length} recebimento(s) foram desfeitos e voltaram para a Agenda.`);
          setBatchRevertingEarnings([]);
        },
        onError: (reason) => setError(getApiErrorMessage(reason, 'Não foi possível desfazer os recebimentos selecionados.')),
      });
    }} />
    <ConfirmationDialog open={batchCancellingEarnings.length > 0} title="Cancelar previsões selecionadas" description={`${batchCancellingEarnings.length} previsão(ões) sairão da Agenda sem criar receita. Você poderá restaurá-las pelo histórico se mudar de ideia.`} confirmLabel="Cancelar selecionadas" busy={batchEarningMutation.isPending} onClose={() => setBatchCancellingEarnings([])} onConfirm={() => {
      setError('');
      batchEarningMutation.mutate({ earningIds: batchCancellingEarnings.map((earning) => earning.id), action: 'CANCEL' }, {
        onSuccess: () => {
          setNotice(`${batchCancellingEarnings.length} previsão(ões) foram canceladas e ficaram disponíveis no histórico.`);
          setBatchCancellingEarnings([]);
        },
        onError: (reason) => setError(getApiErrorMessage(reason, 'Não foi possível cancelar as previsões selecionadas.')),
      });
    }} />
    <ConfirmationDialog open={batchConfirmingEarnings.length > 0} title="Confirmar recebimentos selecionados" description={`${batchConfirmingEarnings.length} provento(s) serão marcados como recebidos e criarão receitas no Histórico Financeiro, alterando o saldo. Se algo estiver incorreto, cada recebimento poderá ser desfeito pelo histórico da Agenda.`} confirmLabel="Confirmar recebimentos" busy={batchEarningMutation.isPending} onClose={() => setBatchConfirmingEarnings([])} onConfirm={() => {
      setError('');
      batchEarningMutation.mutate({ earningIds: batchConfirmingEarnings.map((earning) => earning.id), action: 'CONFIRM' }, {
        onSuccess: () => {
          setNotice(`${batchConfirmingEarnings.length} provento(s) foram confirmados e registrados no Histórico Financeiro.`);
          setBatchConfirmingEarnings([]);
        },
        onError: (reason) => setError(getApiErrorMessage(reason, 'Não foi possível confirmar os recebimentos selecionados.')),
      });
    }} />
  </>;
}

function WalletEarningHistoryActions({ earnings, onRevert, onRestore, onBatchRevert, onBatchRestore, busy }: { earnings: WalletEarningResponse[]; onRevert: (earning: WalletEarningResponse) => void; onRestore: (earning: WalletEarningResponse) => void; onBatchRevert: (earnings: WalletEarningResponse[]) => void; onBatchRestore: (earnings: WalletEarningResponse[]) => void; busy: boolean }) {
  const reversible = earnings.filter((earning) => earning.status === 'EFETIVADO' || earning.status === 'CANCELADO');
  const [selectedIds, setSelectedIds] = useState<number[]>([]);
  const [statusFilter, setStatusFilter] = useState<'TODOS' | 'EFETIVADO' | 'CANCELADO'>('TODOS');
  const [visibleCount, setVisibleCount] = useState(20);
  useEffect(() => setSelectedIds((current) => current.filter((id) => reversible.some((earning) => earning.id === id))), [earnings]);
  useEffect(() => setVisibleCount(20), [statusFilter, earnings]);
  if (reversible.length === 0) return null;
  const filtered = reversible.filter((earning) => statusFilter === 'TODOS' || earning.status === statusFilter);
  const visible = filtered.slice(0, visibleCount);
  const selected = filtered.filter((earning) => selectedIds.includes(earning.id));
  const confirmed = selected.filter((earning) => earning.status === 'EFETIVADO');
  const cancelled = selected.filter((earning) => earning.status === 'CANCELADO');
  const toggle = (id: number) => setSelectedIds((current) => current.includes(id) ? current.filter((item) => item !== id) : [...current, id]);
  const allVisibleSelected = visible.length > 0 && visible.every((earning) => selectedIds.includes(earning.id));
  const selectVisible = () => setSelectedIds((current) => allVisibleSelected
    ? current.filter((id) => !visible.some((earning) => earning.id === id))
    : [...new Set([...current, ...visible.map((earning) => earning.id)])]);
  return <div className="mt-4 rounded-[20px] border border-slate-200 bg-slate-50 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-semibold text-slate-800">Histórico e correções</p><p className="mt-1 text-xs leading-5 text-slate-500">Recebimentos efetivados e previsões canceladas. Selecione somente itens do mesmo estado para corrigir em lote.</p></div><div className="flex rounded-full border border-slate-200 bg-white p-1 text-xs font-semibold"><button className={`rounded-full px-3 py-1.5 ${statusFilter === 'TODOS' ? 'bg-slate-100 text-slate-900' : 'text-slate-500'}`} type="button" onClick={() => setStatusFilter('TODOS')}>Todos ({reversible.length})</button><button className={`rounded-full px-3 py-1.5 ${statusFilter === 'CANCELADO' ? 'bg-slate-100 text-slate-900' : 'text-slate-500'}`} type="button" onClick={() => setStatusFilter('CANCELADO')}>Cancelados</button><button className={`rounded-full px-3 py-1.5 ${statusFilter === 'EFETIVADO' ? 'bg-slate-100 text-slate-900' : 'text-slate-500'}`} type="button" onClick={() => setStatusFilter('EFETIVADO')}>Recebidos</button></div></div><div className="mt-3 flex flex-wrap items-center justify-between gap-3"><label className="flex cursor-pointer items-center gap-2 text-xs font-semibold text-slate-600"><input type="checkbox" checked={allVisibleSelected} onChange={selectVisible} /> Selecionar {visible.length} exibido(s)</label><div className="flex flex-wrap gap-2"><button className="rounded-full border border-rose-200 px-3 py-2 text-xs font-semibold text-rose-700 disabled:border-slate-200 disabled:text-slate-300" type="button" disabled={busy || confirmed.length === 0 || confirmed.length !== selected.length} onClick={() => onBatchRevert(confirmed)}>Desfazer selecionados ({confirmed.length})</button><button className="rounded-full border border-emerald-200 px-3 py-2 text-xs font-semibold text-emerald-700 disabled:border-slate-200 disabled:text-slate-300" type="button" disabled={busy || cancelled.length === 0 || cancelled.length !== selected.length} onClick={() => onBatchRestore(cancelled)}>Restaurar selecionados ({cancelled.length})</button></div></div><div className="mt-3 overflow-x-auto rounded-xl border border-slate-200 bg-white"><table className="w-full min-w-[720px] text-left text-sm"><thead className="bg-slate-50 text-[11px] uppercase tracking-[.08em] text-slate-500"><tr><th className="w-10 px-3 py-3" /><th className="px-3 py-3">Ativo</th><th className="px-3 py-3">Data Com</th><th className="px-3 py-3">Pagamento</th><th className="px-3 py-3 text-right">Líquido</th><th className="px-3 py-3">Estado</th><th className="px-3 py-3 text-right">Ação</th></tr></thead><tbody>{visible.map((earning) => <tr key={earning.id} className="border-t border-slate-100"><td className="px-3 py-3"><input aria-label={`Selecionar ${earning.symbol || earning.assetName}`} type="checkbox" checked={selectedIds.includes(earning.id)} onChange={() => toggle(earning.id)} /></td><td className="px-3 py-3"><b className="text-slate-800">{earning.symbol || earning.assetName}</b><small className="ml-2 text-slate-400">{earning.eventType === 'JCP' ? 'JCP' : earning.eventType === 'RENDIMENTO' ? 'Rendimento' : 'Dividendo'}</small></td><td className="px-3 py-3 text-slate-600">{formatDate(earning.exDate)}</td><td className="px-3 py-3 text-slate-600">{formatDate(earning.paymentDate)}</td><td className="px-3 py-3 text-right font-semibold text-slate-800">{currency(earning.netAmount)}</td><td className="px-3 py-3"><WalletEarningBadge status={earning.status} /></td><td className="px-3 py-3 text-right">{earning.status === 'EFETIVADO' ? <button className="text-xs font-semibold text-rose-700 hover:text-rose-900 disabled:text-slate-300" type="button" disabled={busy} onClick={() => onRevert(earning)}>Desfazer</button> : <button className="text-xs font-semibold text-emerald-700 hover:text-emerald-900 disabled:text-slate-300" type="button" disabled={busy} onClick={() => onRestore(earning)}>Restaurar</button>}</td></tr>)}</tbody></table></div>{filtered.length > visible.length && <button className="mt-3 w-full rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-600 hover:border-emerald-300 hover:text-emerald-700" type="button" onClick={() => setVisibleCount((count) => count + 20)}>Mostrar mais 20 de {filtered.length}</button>}</div>;
}

function WalletEarningAgendaBatchActions({ earnings, onCancel, onConfirm, busy }: { earnings: WalletEarningResponse[]; onCancel: (earnings: WalletEarningResponse[]) => void; onConfirm: (earnings: WalletEarningResponse[]) => void; busy: boolean }) {
  const [selectedIds, setSelectedIds] = useState<number[]>([]);
  useEffect(() => setSelectedIds((current) => current.filter((id) => earnings.some((earning) => earning.id === id))), [earnings]);
  const selected = earnings.filter((earning) => selectedIds.includes(earning.id));
  const confirmable = selected.filter((earning) => earning.status === 'PENDENTE_CONCILIACAO');
  const toggle = (id: number) => setSelectedIds((current) => current.includes(id) ? current.filter((item) => item !== id) : [...current, id]);
  const allSelected = selectedIds.length === earnings.length;
  return <div className="mb-3 rounded-[20px] border border-amber-200 bg-amber-50/60 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-semibold text-amber-950">Ações em lote da Agenda</p><p className="mt-1 text-xs leading-5 text-amber-900">Para confirmar recebimentos, selecione somente itens com estado "A confirmar". A confirmação cria receitas e altera o saldo.</p></div><label className="flex cursor-pointer items-center gap-2 text-xs font-semibold text-amber-900"><input type="checkbox" checked={allSelected} onChange={() => setSelectedIds(allSelected ? [] : earnings.map((earning) => earning.id))} /> Selecionar todos</label></div><div className="mt-3 max-h-40 space-y-2 overflow-y-auto pr-1">{earnings.map((earning) => <label key={earning.id} className="flex cursor-pointer items-center justify-between gap-3 rounded-xl bg-white px-3 py-2 text-sm text-slate-700"><span className="flex min-w-0 items-center gap-3"><input type="checkbox" checked={selectedIds.includes(earning.id)} onChange={() => toggle(earning.id)} /><span>{earning.symbol || earning.assetName} · pagamento {formatDate(earning.paymentDate)} · {currency(earning.netAmount)}</span></span><WalletEarningBadge status={earning.status} /></label>)}</div><div className="mt-3 flex flex-wrap gap-2"><button className="rounded-full bg-slate-950 px-3 py-2 text-xs font-semibold text-white disabled:bg-slate-300" type="button" disabled={busy || confirmable.length === 0 || confirmable.length !== selected.length} onClick={() => onConfirm(confirmable)}>Confirmar recebimentos ({confirmable.length})</button><button className="rounded-full border border-rose-200 px-3 py-2 text-xs font-semibold text-rose-700 disabled:border-slate-200 disabled:text-slate-300" type="button" disabled={busy || selected.length === 0} onClick={() => onCancel(selected)}>Cancelar selecionadas ({selected.length})</button></div></div>;
}
...
function CorporateEventPilotPanel() {
  ...
  const [restoredEarningIds, setRestoredEarningIds] = useState<number[]>([]);
  const [error, setError] = useState('');
  const [notice, setNotice] = useState('');
  const preview = previewMutation.data?.events ?? [];
  const coverage = previewMutation.data?.coverage ?? [];
  const providerLabel = accessQuery.data?.provider === 'B3CorporateEventProvider' ? 'B3 experimental' : 'fonte demonstrativa';
  const newEvents = preview.filter((event) => event.status === 'NOVO');
  const ambiguousEvents = preview.filter((event) => event.status === 'AMBIGUO');
  const cancelledEvents = preview.filter((event) => event.status === 'CANCELADO' && event.walletEarningId != null && !restoredEarningIds.includes(event.walletEarningId));
  const ineligibleEvents = preview.filter((event) => event.status === 'INELEGIVEL_NA_DATA_COM');
  const provisionedEvents = preview.filter((event) => event.status === 'JA_PROVISIONADO');
  const awaitingConfirmationEvents = preview.filter((event) => event.status === 'PENDENTE_CONFIRMACAO');
  const confirmedEvents = preview.filter((event) => event.status === 'CONFIRMADO');
  const actionableEvents = newEvents;
  ...
  const loadPreview = () => {
    setError('');
    setNotice('');
    setRestoredEarningIds([]);
    previewMutation.mutate(undefined, {
      onSuccess: (result: CorporateEventPilotPreviewResponse) => setSelectedReferences(result.events.filter((event) => event.status === 'NOVO').map((event) => event.sourceReference)),
  ...
  return <div className="w-full">
    ...
    {(previewMutation.data || error) && <div className="mt-3 max-h-[420px] w-full overflow-y-auto rounded-[20px] border border-amber-200 bg-amber-50/70 p-4 pr-2 text-sm text-slate-700">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-semibold text-amber-900">Agenda de Proventos Experimental</p><p className="mt-1 max-w-2xl text-xs leading-5 text-amber-800">Fonte ativa: {providerLabel}. A elegibilidade é calculada na Data Com, usando as compras e vendas já registradas. Confira valor e datas antes de considerar o recebimento. Nenhuma receita é criada automaticamente.</p></div><span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-amber-800">{newEvents.length} nova(s) previsão(ões)</span></div>
      {coverage.length > 0 && <div className="mt-3 rounded-xl border border-amber-200/80 bg-white/70 p-3"><p className="text-xs font-semibold text-amber-900">Cobertura devolvida pela fonte</p><div className="mt-2 flex max-h-24 flex-wrap gap-2 overflow-y-auto pr-1">{coverage.map((item) => <span key={item.symbol} className="rounded-lg bg-amber-50 px-2.5 py-1.5 text-[11px] text-amber-950"><b>{item.symbol}</b> · {item.eventCount} evento(s) · Data Com {item.earliestExDate ? formatDate(item.earliestExDate) : 'não informada'} a {item.latestExDate ? formatDate(item.latestExDate) : 'não informada'}</span>)}</div><p className="mt-2 text-[11px] leading-4 text-amber-800">A B3 pública costuma limitar a consulta aos últimos 12 meses. Datas anteriores à faixa exibida não foram recebidas nesta atualização.</p></div>}
      ...
      {actionableEvents.length > 0 && <div className="mt-4 space-y-2">{actionableEvents.map((event) => <label key={event.sourceReference} className="flex gap-3 rounded-xl border border-emerald-200 bg-white p-3"><input className="mt-1" type="checkbox" checked={selectedReferences.includes(event.sourceReference)} onChange={() => toggle(event.sourceReference)} /><span className="min-w-0 flex-1"><span className="flex flex-wrap items-center gap-2"><b>{event.symbol}</b><span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-semibold text-emerald-800">Nova previsão</span></span><span className="mt-1 block text-xs text-slate-500">{event.eventType === 'JCP' ? 'JCP' : event.eventType === 'RENDIMENTO' ? 'Rendimento' : 'Dividendo'} · Data Com {formatDate(event.exDate)} · Pagamento {formatDate(event.paymentDate)}</span><span className="mt-1 block text-xs text-slate-600">{formatQuantity(event.quantityEligible)} cotas · {currency(event.netAmount)} líquido previsto{event.eligibilityStartDate ? ` · na carteira desde ${formatDate(event.eligibilityStartDate)}` : ''}</span></span></label>)}</div>}
      {ambiguousEvents.length > 0 && <details className="mt-4 rounded-xl border border-rose-200 bg-rose-50/70 p-3"><summary className="cursor-pointer text-xs font-semibold text-rose-800">Pendências de vinculação para conferência ({ambiguousEvents.length})</summary><p className="mt-2 text-xs leading-5 text-rose-700">A fonte não informou a classe necessária para vincular estes eventos com segurança. Eles não podem ser adicionados à Agenda.</p><div className="mt-2 max-h-32 space-y-2 overflow-y-auto pr-1">{ambiguousEvents.map((event) => <div key={event.sourceReference} className="rounded-lg border border-rose-200 bg-white px-3 py-2"><p className="text-xs font-semibold text-slate-800">Ticker não identificado · Data Com {formatDate(event.exDate)}</p><p className="mt-1 text-xs text-slate-600">{event.reason}</p></div>)}</div></details>}
      {cancelledEvents.length > 0 && <div className="mt-4 rounded-xl border border-slate-200 bg-white/70 p-3"><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-xs font-semibold text-slate-800">Previsões canceladas</p><p className="mt-1 text-xs text-slate-500">Elas não voltam sozinhas para evitar duplicidade. Restaure somente as que quiser revisar de novo.</p></div><span className="rounded-full bg-slate-100 px-2 py-1 text-[11px] font-semibold text-slate-600">{cancelledEvents.length}</span></div><div className="mt-2 space-y-2">{cancelledEvents.map((event) => <div key={event.sourceReference} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2"><span className="text-xs text-slate-700"><b>{event.symbol || event.assetName}</b> · Data Com {formatDate(event.exDate)} · {currency(event.netAmount)}</span><button className="text-xs font-semibold text-emerald-700 disabled:text-slate-300" type="button" disabled={adjustEarningMutation.isPending} onClick={() => { if (event.walletEarningId == null) return; setError(''); setNotice(''); adjustEarningMutation.mutate({ id: event.walletEarningId, data: { reopened: true } }, { onSuccess: () => { setRestoredEarningIds((current) => [...current, event.walletEarningId!]); setNotice('Previsão restaurada na Agenda para nova revisão.'); }, onError: (reason) => setError(getApiErrorMessage(reason, 'Não foi possível restaurar a previsão.')) }); }}>Restaurar</button></div>)}</div></div>}
      {(ineligibleEvents.length > 0 || provisionedEvents.length > 0 || awaitingConfirmationEvents.length > 0 || confirmedEvents.length > 0) && <details className="mt-4 rounded-xl border border-slate-200 bg-white/60 p-3"><summary className="cursor-pointer text-xs font-semibold text-slate-700">Diagnóstico da atualização: {ineligibleEvents.length} inelegível(is), {provisionedEvents.length} provisionada(s), {awaitingConfirmationEvents.length} aguardando confirmação e {confirmedEvents.length} confirmada(s)</summary><p className="mt-2 text-xs leading-5 text-slate-600">Itens sem direito não são erro: a posição não existia ou não tinha cotas na Data Com. Eventos provisionados, pendentes de confirmação e confirmados são mantidos no estado atual e nunca são recriados.</p>{[...ineligibleEvents, ...provisionedEvents, ...awaitingConfirmationEvents, ...confirmedEvents].length > 0 && <div className="mt-2 max-h-32 space-y-1 overflow-y-auto pr-1">{[...ineligibleEvents, ...provisionedEvents, ...awaitingConfirmationEvents, ...confirmedEvents].slice(0, 30).map((event) => <p key={event.sourceReference} className="text-xs text-slate-500"><b>{event.symbol || event.assetName}</b> · Data Com {formatDate(event.exDate)} · {event.reason}</p>)}</div>}</details>}
...
function assetLabel(type: InvestmentAssetType) { return ({ ACAO: 'Ações', FII: 'FIIs', FIAGRO: 'Fiagros', BDR: 'BDRs', ETF: 'ETFs', CRIPTO: 'Cripto', RENDA_FIXA: 'Renda fixa' })[type]; }
